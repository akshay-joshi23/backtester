...
__all__ = [
    ...
    "_parse_defaults",
    "_ruff_check",
    # Deprecated shims — kept until lab/agent.py is refactored onto the
    # provider abstraction (next commit). DO NOT use in new code.
    "_make_client",
    "_call_anthropic",
    "_extract_usage",
]
...
# --------------------------------------------------------------------------- #
# DEPRECATED — backward-compat shims kept only for lab/agent.py until it's
# refactored to use the provider abstraction in the next commit. Will be
# removed thereafter. New code: use get_provider() instead.
# --------------------------------------------------------------------------- #
...
def _call_anthropic(client, messages, *, model, max_tokens, temperature,
                     system_prompt):
